                ...
                <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-800/60">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <span class="rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{{ $item->origin->name }}</span>
                            @svg('heroicon-o-arrow-right', 'h-4 w-4 text-gray-400')
                            <span class="rounded-full bg-indigo-50 px-3 py-1 text-sm font-medium text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300">{{ $item->destination->name }}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-right font-semibold text-gray-900 dark:text-white">{{ $item->distance_km }} km</td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-1">
                            ...
                            @can($rolesName.'.delete')
                                <x-idcore::partials.delete-button :module="$module" :id="$item->id" :name="$item->origin->name.' - '.$item->destination->name" />
                            @endcan
                        </div>
                    </td>
                </tr>
