...
<x-idcore::card title="{{ $subtitle }}" subtitle="{{ $title }}" class="max-w-2xl">
    <form action="{{ $action }}" method="POST" class="space-y-6"
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <x-idcore::input name="name" label="Nama" required :value="$formData->name ?? null" placeholder="Contoh: Basic" />
            <x-idcore::input name="price" label="Harga (Rp)" type="number" step="0.01" min="0" required :value="$formData->price ?? null" placeholder="0" />
            <x-idcore::input name="max_products" label="Maks Produk" type="number" min="0" :value="$formData->max_products ?? null" placeholder="Kosongkan jika tidak terbatas" />
            <x-idcore::input name="max_discount" label="Maks Diskon (%)" type="number" step="0.01" min="0" max="100" :value="$formData->max_discount ?? null" placeholder="0" />
        ...
        <x-idcore::toggle name="can_run_campaign" label="Dapat menjalankan campaign" :checked="$formData->can_run_campaign ?? false" />
