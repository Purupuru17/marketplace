...
<div x-data="{ on: @js((bool) old($name, $checked)) }" class="flex items-center gap-3">
    ...
    <button type="button" @click="on = !on" :aria-pressed="on"
            :class="on ? 'bg-brand-600' : 'bg-gray-200 dark:bg-gray-700'"
            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 dark:focus:ring-offset-gray-950">
