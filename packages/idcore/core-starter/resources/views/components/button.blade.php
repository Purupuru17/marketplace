...
@php
    ...
    $sizeClasses = match($size) {
        'xs' => $circle ? 'h-7 w-7 p-0 text-xs' : 'py-1.5 px-2 text-xs',
        'sm' => $circle ? 'h-9 w-9 p-0 text-xs' : 'py-2 px-6 text-xs',
        'lg' => $circle ? 'h-12 w-12 p-0 text-base' : 'py-4 px-10 text-base',
        default => $circle ? 'h-10 w-10 p-0 text-sm' : 'py-3 px-8 text-sm',
    };

    $baseClasses = 'inline-flex items-center justify-center gap-2.5 text-center font-medium duration-200 ease-in-out rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50';
...
@if($href)
    <a href="{{ $loading ? '#' : $href }}"
       {{ $attributes->merge(['class' => "$baseClasses $variantClasses $sizeClasses $shapeClasses $widthClasses $loadingClasses"]) }}>
        @if($loading)
...
@else
    <button type="{{ $type }}"
            {{ $attributes->merge(['class' => "$baseClasses $variantClasses $sizeClasses $shapeClasses $widthClasses"]) }}
