...
<div class="flex flex-col items-center justify-between gap-3 border-t border-gray-100 px-5 py-4 text-sm dark:border-gray-800 sm:flex-row">
    ...
    <div class="flex items-center gap-1">
        @foreach($paginator->linkCollection() as $link)
            ...
            @elseif($link['active'])
                <span class="inline-flex h-9 min-w-9 items-center justify-center rounded-lg bg-brand-600 px-3 text-xs font-semibold text-white">{!! $label !!}</span>
            @else
                <a href="{{ $link['url'] }}" class="inline-flex h-9 min-w-9 items-center justify-center rounded-lg border border-gray-200 bg-white px-3 text-xs font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800">{!! $label !!}</a>
            @endif
        @endforeach
    </div>
</div>
