...
class="overflow-hidden {{ $embedded ? '' : 'rounded-2xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]' }}">
    ...
    <div class="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 dark:border-gray-800 md:flex-row md:items-center md:justify-between">
        <div class="flex flex-wrap items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            <span>Show</span>
            <x-idcore::select name="dt-per-page" x-model="perPage" @change="page = 1" :options="collect($perPageOptions)->mapWithKeys(fn($o) => [$o => $o])->all()" placeholder="" class="!w-20" />
            <span>entries</span>
            @isset($filters)
                <span class="mx-1 hidden h-4 w-px bg-gray-200 dark:bg-gray-700 sm:block"></span>
                {{ $filters }}
            @endisset
        </div>
        <div class="w-full md:max-w-xs">
            <x-idcore::input x-model="search" type="search" name="dt-search" icon="magnifying-glass" placeholder="Search..." class="pr-8" />
        </div>
    </div>
    @endif
